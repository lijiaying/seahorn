...
#define mybold "\e[1m"
	...
	bool HornifyModule::runOnModule (Module &M)
	{
		...
		errs () << myyellow << " --> Hornify Module runOnModule\n" << mywhite;
		...
		if (Step == hm_detail::CLP_SMALL_STEP || 
			...
			errs () << myred << "-> ufo small sym exec\n" << mywhite;
			errs () << "m_expr_factory: " << m_efac << "\n";
		...
		if (!canFail)
		{
			errs () << mygreen << " -> can not fail @" << __FILE__ << " L" << __LINE__ << "\n" << mywhite;
		...
		if (!NoVerification)
		{
			errs () << mygreen << " -> No verification @" << __FILE__ << " L" << __LINE__ << "\n" << mywhite;
		...
		{
			errs () << mygreen << " -> Get to verifier.error @" << __FILE__ << " L" << __LINE__ << "\n" << mywhite;
		...
		for (auto it = scc_begin (&CG); !it.isAtEnd (); ++it)
		{
			...
			// errs () << myblue << "try to runOnFunction\n";
			if (f) Changed = (runOnFunction (*f) || Changed);
		}
		errs () << myyellow << "  <-- finish SCC \n" << mywhite;

		if (!m_db.hasQuery ())
		{
			errs () << myyellow << "  --> not has query\n" << mywhite;
...
*/
		errs () << myyellow << " <-- HornifyModule runOnModule\n" << mywhite;
		return Changed;
	}
	...
	bool HornifyModule::runOnFunction (Function &F)
	{
		...
		errs () << mygreen << mybold << " >>>>  runOnFunction [" << F.getName() << "] --------------------------- ##L " << __LINE__ << "-------------------------------\n" << mywhite << m_db << "\n";
		LOG("horn-step", errs () << "HornifyModule: runOnFunction: " << F.getName () << "\n");
		// errs () << myred << " --> HornifyFunction runOnFunction [" << F.getName() << "]\n" << mywhite;
		...
		boost::scoped_ptr<HornifyFunction> hf (new SmallHornifyFunction(*this, InterProc));
		...
		// errs () << myred << " <-- HornifyFunction runOnFunction [" << F.getName() << "]\n" << mywhite;
		errs () << myblue << mybold << " <<<<  runOnFunction [" << F.getName() << "] --------------------------- ##L " << __LINE__ << "-------------------------------\n" << mywhite << m_db << "\n";
		return false;
	}
